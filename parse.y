...
//inbuilt functions AND STATEMENTS FOR DF AND MATRIX
//create a dataframe
...
//DELETE A DF ENTRIES BASED ON PREDICATE EX: delete(df,col[i][j]==3);
DF_DELETEROW : ID OBRAK ID COMMA PREDICATE CBRAK SEMICOL
             ;

//SELECT A SUBSET OF DF 
DF_SELECT   : ID OBRAK ID COMMA ID COMMA PREDICATE CBRAK SEMICOL
            ;
...
//
//UPDATE DF ENTRIES
DF_UPDATECOL : ID OBRAK ID COMMA ID COMMA PREDICATE COMMA rhs CBRAK SEMICOL
             | ID OBRAK ID COMMA ID COMMA NULL COMMA rhs CBRAK SEMICOL
             ;
